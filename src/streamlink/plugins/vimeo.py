...
class VimeoHLSStream(HLSStream):
    ...
    @property
    def url(self):
        if self._need_update and self.api.updating.acquire(blocking=False):
            try:
            ...
            self._need_update = False
            self.api.updating.release()

        if self.api.updating.acquire(blocking=False):
            time_now = time()
            if time_now > self.api.expiry_time:
                if time_now >= self.api.last_updated + self.api.EXPIRY_TIME_LIMIT:
...
class Vimeo(Plugin):
    ...
    def _get_streams(self):
        ...
        if not data:
            return
        if "error" in data:
            log.error(f"{data['title']} {data['message']}")
            return
